...
namespace OpenXmlPowerTools
{
    internal class Program
    {
        private static void Main()
        {
            ...
            foreach (FileInfo file in sourceDi.GetFiles("*.docx"))
            {
                File.Copy(file.FullName, Path.Combine(tempDi.FullName, file.Name));
            }

            foreach (FileInfo file in sourceDi.GetFiles("*.pptx"))
            {
                File.Copy(file.FullName, Path.Combine(tempDi.FullName, file.Name));
            }

            string[] fileList = Directory.GetFiles(tempDi.FullName, "*.docx");
            foreach (string file in fileList)
            {
                var fi = new FileInfo(file);
                Console.WriteLine(fi.Name);

                string newFileName = "Updated-" + fi.Name;
                var fi2 = new FileInfo(Path.Combine(tempDi.FullName, newFileName));
                File.Copy(fi.FullName, fi2.FullName);

                using WordprocessingDocument wDoc = WordprocessingDocument.Open(fi2.FullName, true);
            ...
            foreach (string file in fileList)
            {
                ...
                using PresentationDocument pDoc = PresentationDocument.Open(fi2.FullName, true);
